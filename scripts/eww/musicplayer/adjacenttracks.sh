...
if [ -e $isrunningqmmp ]; then
    ...
    if [ "$prev_index" -lt 1 ] || [ "$current_index" -eq 0 ]; then
        ...
        prev_song=$(echo "$prev_line" | sed 's/^[0-9]*\. //' | sed -E 's|^[0-9]+ - ||; s|\[.*\]||g; s|\.mp3||; s|\.wav||; s|  +| |g; s/\|.*//g; s|&|\&|g; s/｜.*//g; s|\(Official.*||g; s| - [0-9]+:.*||')
    fi
    ...
    if [ "$next_index" -gt "$total_tracks" ] || [ "$current_index" -eq 0 ]; then
        ...
        next_song=$(echo "$next_line" | sed 's/^[0-9]*\. //' | sed -E 's|^[0-9]+ - ||; s|\[.*\]||g; s|\.mp3||; s|\.wav||; s|  +| |g; s/\|.*//g; s|&|\&|g; s/｜.*//g; s|\(Official.*||g; s| - [0-9]+:.*||')
    fi
else
    prev_song=""
    next_song=""
